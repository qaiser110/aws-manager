...
class DynamoManager {
  ...
  async delete(keys) {
    if (!Array.isArray(keys) || !keys.length) {
      throw new Error(
        'Array of keys to delete in DynamoManager.delete() is required.'
      )
    }
    const params = {
      Bucket: this.bkt,
  ...
  async deleteByPrefix(prefix) {
    if (typeof prefix !== 'string') {
      throw new Error('prefix in DynamoManager.deleteByPrefix() must be String.')
    }
  ...
  async tagObjects(objectKeys, tagKey, tagValue) {
    ...
    if (!(tagKey && tagValue))
      throw new Error(
        'tagKey and tagValue are required in DynamoManager.putObjectTags()'
      )
